...
export default function CourseDetailsPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col font-sans bg-section text-text">
      ...
      <div className="w-full bg-card border-b border-border pt-32 pb-12">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex items-center gap-2 text-[13px] font-semibold text-muted/70 mb-6">
            ...
            <Link href="/user/training/courses" className="hover:text-heading transition-colors">Courses</Link>
            <ChevronRight size={14} />
              ...
              <div className="flex flex-wrap items-center gap-6 mt-2">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-[13px] font-semibold text-heading">{course.instructor}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex text-yellow-400">
                    ...
                    <Star size={16} fill="currentColor" />
                    <Star size={16} fill="currentColor" />
                    <Star size={16} className="text-heading/20" />
                  </div>
      ...
      <div className="flex-grow pt-8 pb-24 px-4 sm:px-6 lg:px-8 max-w-[1400px] mx-auto w-full relative">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 relative">
              ...
              {['overview', 'curriculum', 'instructor', 'reviews'].map((tab) => (
                <button 
                  ...
                  className={`pb-4 text-[15px] font-bold capitalize transition-all relative ${
                    activeTab === tab ? 'text-accent' : 'text-muted/70 hover:text-heading'
                  }`}
                >
                  <div className="flex items-center gap-2">
            ...
            <div className="min-h-[400px]">
              ...
              {activeTab === 'overview' && (
                <div className="animate-in fade-in duration-300">
                  <h3 className="text-2xl font-bold text-heading mb-6">Course Description</h3>
                  <p className="text-[15px] text-muted/90 leading-relaxed mb-8">
                    {course.overview}
                  </p>
                  
                  <div className="bg-card border border-border p-8 rounded-3xl">
                    <h4 className="text-[17px] font-bold text-heading mb-6">What you'll learn</h4>
              ...
              {activeTab === 'curriculum' && (
                <div className="animate-in fade-in duration-300">
                  <h3 className="text-2xl font-bold text-heading mb-6">Curriculum</h3>
                  <div className="flex flex-col gap-4">
                    {course.curriculum.map((section, i) => (
                      <div key={i} className="bg-card border border-border rounded-2xl overflow-hidden">
                        <div className="p-5 border-b border-border bg-white/[0.02]">
                          <h4 className="text-[16px] font-bold text-heading">{section.section}</h4>
                        </div>
                        <div className="p-2">
                          {section.lessons.map((lesson, j) => (
                            <div key={j} className="flex items-center justify-between p-3 hover:bg-white/5 rounded-xl transition-colors cursor-pointer group">
                              <div className="flex items-center gap-3 text-[14px] font-medium text-muted/90 group-hover:text-heading">
              ...
              {activeTab === 'instructor' && (
                <div className="animate-in fade-in duration-300">
                  <h3 className="text-2xl font-bold text-heading mb-6">About the Instructor</h3>
                  <div className="flex flex-col sm:flex-row gap-8 items-start">
                    ...
                    <div>
                      <h4 className="text-xl font-bold text-heading mb-1">{course.instructor}</h4>
              ...
              {activeTab === 'reviews' && (
                <div className="animate-in fade-in duration-300">
                  <h3 className="text-2xl font-bold text-heading mb-6">Student Feedback</h3>
                  
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-10">
                     <div className="md:col-span-4 bg-card border border-border rounded-3xl p-8 flex flex-col items-center justify-center text-center">
                       <div className="text-6xl font-black text-heading mb-2">{course.rating}</div>
                       <div className="flex text-yellow-400 mb-2">
                          <Star size={20} fill="currentColor" />
                          <Star size={20} fill="currentColor" />
                          <Star size={20} fill="currentColor" />
                          <Star size={20} fill="currentColor" />
                          <Star size={20} className="text-heading/20" />
                        </div>
                     ...
                     <div className="md:col-span-8 flex flex-col justify-center gap-3">
                        {[5, 4, 3, 2, 1].map((star) => (
                           <div key={star} className="flex items-center gap-4">
                             <div className="flex items-center gap-1 w-12 text-[13px] font-bold text-heading">
                    ...
                    {[1, 2, 3].map((_, i) => (
                      <div key={i} className="border-b border-border pb-6">
                        <div className="flex items-center gap-4 mb-3">
                          <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-[13px] font-bold text-heading">
                            U{i+1}
                          </div>
                          <div>
                            <div className="text-[14px] font-bold text-heading">User {i+1}</div>
                            <div className="flex text-yellow-400">
          ...
          <div className="lg:col-span-1">
            <div className="bg-card rounded-3xl border border-border shadow-2xl sticky top-28 overflow-hidden lg:-mt-64 relative z-20">
              ...
              <div className="relative aspect-video bg-black cursor-pointer group">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                   <div className="w-16 h-16 rounded-full bg-accent/90 flex items-center justify-center text-heading shadow-[0_0_30px_rgba(0,184,219,0.5)] mb-3 group-hover:scale-110 transition-transform">
                      <Play size={24} className="ml-1" />
                   </div>
                   <span className="text-[14px] font-bold text-heading tracking-widest uppercase">Preview this course</span>
                </div>
              </div>
              ...
              <div className="p-8">
                ...
                <div className="flex flex-col gap-3 mb-6">
                  <button className="w-full h-14 bg-gradient-to-r from-accent to-accent-cool text-page text-[15px] font-extrabold rounded-xl shadow-[0_0_20px_rgba(0,184,219,0.3)] hover:shadow-[0_0_30px_rgba(0,184,219,0.5)] transition-all hover:-translate-y-0.5">
                    Enroll now
                  </button>
                  <button className="w-full h-14 bg-white/5 border border-border text-heading text-[15px] font-bold rounded-xl hover:bg-white/10 transition-colors">
                    Add to cart
                  </button>
                </div>
                ...
                {/* Includes */}
                <h4 className="text-[14px] font-bold text-heading mb-4">This course includes:</h4>
                ...
                <div className="flex items-center justify-between border-t border-border pt-6">
                  <button className="flex items-center gap-2 text-[13px] font-semibold text-heading hover:text-accent transition-colors"><Share2 size={16} /> Share</button>
                  <button className="flex items-center gap-2 text-[13px] font-semibold text-heading hover:text-accent transition-colors"><Heart size={16} /> Wishlist</button>
                </div>
