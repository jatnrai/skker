...
export default function TrainingWorkflow() {
  ...
  return (
    <section id="private-training" className="mb-32">
      <div className="grid grid-cols-1 lg:grid-cols-[0.85fr_1.15fr] gap-6 lg:gap-8 items-stretch">
        
        {/* Left: How to Choose */}
        <div className="bg-section/50 border border-border rounded-[28px] p-8 sm:p-10 flex flex-col justify-center relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <Link 
              ...
              className="inline-flex items-center justify-center px-8 py-3.5 bg-white/5 border border-border hover:border-accent/40 rounded-full text-heading font-mono text-[10px] font-bold tracking-[0.1em] uppercase hover:bg-white/10 transition-all hover:text-accent gap-3 group/btn"
            >
        ...
        <div className="bg-section/50 border border-border rounded-[28px] p-8 sm:p-10">
          <div className="font-mono text-[10px] font-bold tracking-[0.2em] uppercase text-accent/80 mb-4">
            Corporate workflow
          </div>
          <h3 className="text-[32px] sm:text-[40px] font-serif font-bold text-heading leading-[1.05] tracking-tight mb-8">
            From requirement to proposal.
          </h3>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {steps.map((step, index) => (
              <div key={index} className="bg-white/[0.03] border border-border rounded-[20px] p-5 flex flex-col h-full hover:bg-white/[0.05] transition-colors">
                <span className="font-mono text-[10px] font-bold tracking-[0.15em] text-accent mb-4 block">
                  {step.num}
                </span>
                <strong className="text-[15px] text-heading font-medium leading-[1.3] block mb-3 h-[40px]">
                  {step.title}
                </strong>
