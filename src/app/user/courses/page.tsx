  ...
  const RadioCustom = ({ checked, label, count }: { checked: boolean, label: string, count?: number }) => (
    <div className="flex items-center justify-between w-full cursor-pointer group">
      <div className="flex items-center gap-3">
        <div className={`w-[18px] h-[18px] rounded-full border-2 flex items-center justify-center transition-colors ${checked ? 'border-accent' : 'border-white/20 group-hover:border-white/40'}`}>
          {checked && <div className="w-2.5 h-2.5 rounded-full bg-accent shadow-[0_0_8px_rgba(0,184,219,0.8)]" />}
        </div>
        <span className={`text-[13px] transition-colors ${checked ? 'text-white font-semibold' : 'text-muted/80 group-hover:text-white'}`}>
          {label}
        </span>
      </div>
      {count !== undefined && (
      ...
      <div className="flex-grow pt-28 pb-24 px-4 sm:px-6 lg:px-8 max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="w-full lg:w-[280px] shrink-0">
            <div className="bg-[#0b1622] rounded-2xl border border-white/5 p-6 sticky top-28 flex flex-col gap-8 shadow-xl">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full h-[42px] bg-[#060e15] border border-white/5 rounded-xl pl-10 pr-4 text-sm text-white placeholder:text-muted/50 outline-none focus:border-accent/50 transition-all"
                />
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <h4 className="text-[14px] font-bold text-white">Categories</h4>
                  <span className="bg-[#112435] text-accent text-[10px] font-bold px-1.5 py-0.5 rounded">6</span>
                </div>
              ...
              {/* Price */}
              <div>
                <h4 className="text-[14px] font-bold text-white mb-4">Price</h4>
                <div className="flex flex-col gap-4">
                  {PRICES.map(price => (
              ...
              {/* Level */}
              <div>
                <h4 className="text-[14px] font-bold text-white mb-4">Level</h4>
                <div className="flex flex-col gap-4">
                  {LEVELS.map(level => (
              ...
              {/* Language */}
              <div>
                <h4 className="text-[14px] font-bold text-white mb-4">Language</h4>
                <div className="flex flex-col gap-4">
                  {LANGUAGES.map(lang => (
              ...
              {/* Ratings */}
              <div>
                <h4 className="text-[14px] font-bold text-white mb-4">Ratings</h4>
                <div className="flex flex-col gap-4">
                  {RATINGS.map(rating => (
              ...
              <button className="w-full mt-2 h-11 rounded-xl border border-white/10 bg-[#060e15] text-[13px] font-semibold text-muted/80 hover:text-white hover:border-white/20 transition-all flex items-center justify-center gap-2">
                Clear all filters
              </button>
            </div>
          </aside>
          ...
          <div className="flex-1 min-w-0 w-full flex flex-col gap-6">
            ...
            {/* Top Bar */}
            <div className="bg-[#0b1622] border border-white/5 p-4 rounded-2xl flex flex-col sm:flex-row gap-4 items-center justify-between shadow-sm">
              <div className="relative w-full sm:w-[320px]">
                ...
                <input 
                  ...
                  className="w-full h-10 bg-[#060e15] border border-white/5 rounded-xl pl-10 pr-4 text-sm text-white placeholder:text-muted/50 outline-none focus:border-accent/50 transition-all"
                />
              </div>
              ...
              <div className="flex items-center gap-3">
                <select 
                  ...
                  className="h-10 bg-[#060e15] border border-white/5 rounded-xl px-4 text-[13px] text-white font-medium outline-none focus:border-accent/50 appearance-none min-w-[160px] cursor-pointer"
                >
                ...
                <div className="flex items-center gap-1 bg-[#060e15] border border-white/5 p-1 rounded-xl">
                  ...
                  <button className="w-8 h-8 rounded-lg text-muted/60 hover:text-white flex items-center justify-center transition-colors">
                    <List size={16} />
                  </button>
                </div>
                
                <button className="w-10 h-10 rounded-xl border border-white/5 bg-[#060e15] text-muted/60 hover:text-white flex items-center justify-center transition-colors">
                  <RotateCcw size={16} />
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredCourses.map(course => (
                <div key={course.id} className="group flex flex-col bg-[#0b1622] rounded-3xl border border-white/5 overflow-hidden transition-all duration-300">
                  ...
                  <div className="relative h-[200px] overflow-hidden">
                    ...
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0b1622] via-transparent to-transparent opacity-90" />
                    ...
                    <div className="absolute top-4 left-4 flex justify-between right-4">
                      <span className={`px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-widest rounded-full ${
                        course.level === 'Advanced' ? 'bg-[#ff6b6b] text-white shadow-[0_0_10px_rgba(255,107,107,0.4)]' 
                        : course.level === 'Beginner' ? 'bg-[#20e2a8] text-[#04121d] shadow-[0_0_10px_rgba(32,226,168,0.4)]'
                        : 'bg-[#feca57] text-[#04121d] shadow-[0_0_10px_rgba(254,202,87,0.4)]'
                      }`}>
                        {course.level}
                      </span>
                      
                      <button className="w-8 h-8 rounded-full bg-black/40 backdrop-blur-md border border-white/10 flex items-center justify-center text-white/70 hover:text-white hover:bg-black/60 transition-all">
                        <Heart size={14} />
                      </button>
                    </div>

                    {/* Bottom Info Overlay */}
                    <div className="absolute bottom-3 left-4 right-4 flex items-center gap-4 text-[11px] font-bold text-white/90">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <h3 className="text-[17px] font-bold text-white mb-3 leading-snug">
                      <Link href={`/user/courses/${course.id}`} className="hover:text-accent transition-colors">
                        {course.title}
                      </Link>
                    </h3>
                    ...
                    {course.isNew && (
                      <div className="mb-6">
                         <span className="px-2.5 py-1 text-[10px] font-bold text-white/70 bg-white/5 border border-white/10 rounded-full">
                           NEW
                         </span>
                      </div>
                    )}
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="px-2 py-0.5 text-[10px] font-bold text-white bg-[#ff4757] rounded-md ml-1 shadow-[0_0_10px_rgba(255,71,87,0.3)]">
                            -{course.discount}%
                          </span>
                        </div>
                        
                        <Link href={`/user/courses/${course.id}`} className="h-10 px-5 bg-gradient-to-r from-accent to-accent-cool text-[#04121d] text-[13px] font-bold rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(0,184,219,0.2)] hover:shadow-[0_0_25px_rgba(0,184,219,0.4)] transition-all">
                          Enroll now
                        </Link>
                      </div>
                      ...
                      <div className="flex items-center gap-6 pt-5 border-t border-white/5">
                        <button className="flex items-center gap-2 text-[12px] font-semibold text-muted/70 hover:text-white transition-colors">
