...
export default function WhereIOperateSection() {
  ...
  return (
    <section id="where-operate" className="py-24 sm:py-32 px-4 sm:px-6 relative z-10 bg-[#06091a]">
      ...
      <div className="max-w-[1080px] mx-auto w-full grid grid-cols-1 md:grid-cols-2 gap-12 sm:gap-20 items-start relative z-10">
        ...
        {/* Left Column (Media) */}
        <div className="relative w-full aspect-square md:aspect-[4/5] rounded-[32px] overflow-hidden border border-white/5 shadow-2xl">
          <img 
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#06091a]/90 via-transparent to-transparent"></div>
          
          <div className="absolute bottom-6 left-6 right-6 bg-[#0a1520]/80 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-xl">
             <div className="font-serif font-bold text-white text-xl mb-1">Where I operate</div>
        ...
        <div className="flex flex-col pt-4 md:pt-10">
          ...
          <h2 className="text-[35px] sm:text-[45px] font-serif font-bold text-[#eef0f8] leading-[1.08] tracking-[-0.025em] mb-6">
          ...
          <div className="flex flex-col gap-4">
            {cards.map((card, idx) => (
              <div 
                key={idx}
                className="group flex gap-5 bg-card-bg/40 hover:bg-card-hover-bg/80 border border-white/5 rounded-2xl p-6 transition-all duration-300"
              >
                <div className="font-mono text-[11px] font-bold tracking-[0.1em] text-accent mt-1">
                  {card.num}
                </div>
                <div className="flex flex-col">
                  <strong className="text-[14.5px] font-bold text-[#eef0f8] mb-1 group-hover:text-white transition-colors">{card.title}</strong>
