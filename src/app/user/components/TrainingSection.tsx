...
export default function TrainingSection() {
  ...
  return (
    <section id="training" className="py-24 sm:py-32 px-4 sm:px-6 relative z-10 bg-[#040816]">
      <div className="max-w-[1200px] mx-auto w-full relative z-10 flex flex-col gap-16">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-end">
          ...
          {/* Center: Main Image */}
          <motion.div {...fadeUp} className="lg:col-span-5 h-[200px] sm:h-[280px] rounded-[32px] overflow-hidden border border-white/5 relative">
            <img 
              src="https://images.unsplash.com/photo-1524178232363-1fb2b075b655?auto=format&fit=crop&w=1600&q=80" 
              alt="Training Workshop" 
              className="w-full h-full object-cover opacity-80 hover:opacity-100 transition-opacity duration-500" 
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#040816] to-transparent mix-blend-multiply opacity-50"></div>
          </motion.div>
        ...
        >
          {trainingCards.map((card, idx) => (
            ...
            >
              <Link 
                href={card.href}
                className="group flex items-center justify-between gap-6 p-6 sm:p-8 bg-card-bg/30 border border-white/5 hover:border-white/10 hover:bg-card-hover-bg rounded-[32px] transition-all duration-300 relative overflow-hidden"
              >
                {/* Text Content */}
                <div className="flex flex-col z-10 flex-grow">
                  ...
                  <h3 className="font-serif text-[18px] sm:text-[20px] text-white font-bold group-hover:text-[#a2f4fd] transition-colors">
                    {card.title}
                  </h3>
                </div>
                ...
                <div className="flex items-center gap-4 z-10 shrink-0">
                  <div className="w-[100px] h-[60px] sm:w-[130px] sm:h-[75px] rounded-2xl overflow-hidden border border-white/10 relative">
                    <img 
                      src={card.img} 
                      alt={card.title} 
