...
import { Search, Plus, PlayCircle, Eye, Edit, Trash2 } from 'lucide-react';
import Link from 'next/link';
import CourseActionMenu from '../components/CourseActionMenu';

export default function SelfPacedCourses() {
  ...
  async function fetchCourses() {
    try {
      ...
      if (res.ok) {
        ...
        setCourses(data.courses?.filter((c: any) => c.trainingType === 'Self-Paced') || []);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const deleteCourse = async (id: string) => {
    if (!confirm("Are you sure you want to delete this self-paced course?")) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-admin-text">Self-Paced Courses</h1>
          <p className="text-admin-muted text-sm mt-1">Manage recorded learning and course modules.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <Plus size={16} /> New Self-Paced Course
          </Link>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-admin-border/50">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-admin-muted">No self-paced courses found.</td>
                </tr>
              ) : courses.map(course => (
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-admin-bg border border-admin-border flex items-center justify-center shrink-0">
                        <PlayCircle size={16} className="text-admin-muted" />
                      </div>
                      <div>
