...
import { Search, Plus, Briefcase, Eye, Edit, Trash2 } from 'lucide-react';
...
export default function CorporateTraining() {
  ...
  async function fetchCourses() {
    try {
      ...
      if (res.ok) {
        ...
        setCourses(data.courses?.filter((c: any) => c.trainingType === 'Corporate') || []);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const deleteCourse = async (id: string) => {
    if (!confirm("Are you sure you want to delete this corporate training module?")) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-admin-text">Corporate Training</h1>
          <p className="text-admin-muted text-sm mt-1">Manage corporate training catalogues and engagements.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <Plus size={16} /> New Corporate Module
          </Link>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-admin-bg/50 border-b border-admin-border text-xs font-semibold text-admin-muted uppercase tracking-wider">
                <th className="p-4">Catalogue Module</th>
            ...
            <tbody className="divide-y divide-admin-border/50">
              {isLoading ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-admin-muted">Loading...</td>
                </tr>
              ) : courses.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-admin-muted">No corporate modules found.</td>
                </tr>
              ) : courses.map(course => (
                <tr key={course.id} className="hover:bg-admin-bg/20 transition-colors">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-admin-bg border border-admin-border flex items-center justify-center shrink-0">
                        <Briefcase size={16} className="text-admin-muted" />
                      </div>
                      <div>
                  ...
                  <td className="p-4 text-sm text-admin-text font-medium">
                    {course.price === '0' || !course.price ? 'Custom Quote' : `RM ${course.price}`}
                  </td>
                  <td className="p-4">
