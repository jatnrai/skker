...
export default function PrivateTraining() {
  ...
  async function fetchCourses() {
    try {
      ...
      if (res.ok) {
        ...
        setCourses(data.courses?.filter((c: any) => c.trainingType === 'Private') || []);
      }
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const deleteCourse = async (id: string) => {
    if (!confirm("Are you sure you want to delete this private training offering?")) return;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-admin-text">Private Training</h1>
          <p className="text-admin-muted text-sm mt-1">Manage 1-on-1 and private group offerings.</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          >
            <Plus size={16} /> New Private Offering
          </Link>
        </div>
      </div>
      ...
      <div className="bg-admin-surface border border-admin-border rounded-xl overflow-hidden flex flex-col min-h-[400px]">
        <div className="p-4 border-b border-admin-border flex items-center justify-between shrink-0">
          <div className="relative w-64">
            ...
            <input 
              type="text" 
              placeholder="Search private offerings..." 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-admin-bg/50 border-b border-admin-border text-xs font-semibold text-admin-muted uppercase tracking-wider">
                ...
                <th className="p-4">Hourly/Session Rate</th>
            ...
            <tbody className="divide-y divide-admin-border/50">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-admin-muted">No private training offerings found.</td>
                </tr>
              ) : courses.map(course => (
